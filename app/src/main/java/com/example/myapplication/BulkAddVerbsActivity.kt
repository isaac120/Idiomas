...
class BulkAddVerbsActivity : AppCompatActivity() {
    ...
    private lateinit var adapter: VerbRowAdapter

    private val database by lazy { AppDatabase.getDatabase(this) }
    private var listId: Long = -1
    private var listName: String = ""
    private val rows = mutableListOf<VerbRow>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_bulk_add_verbs)
    ...
    private fun updateSaveButton() {
        val validCount = adapter.getValidVerbs().size
        saveButton.text = "💾 Guardar ($validCount verbos)"
        saveButton.isEnabled = validCount > 0
    }

    private fun saveVerbs() {
        val validVerbs = adapter.getValidVerbs()

        if (validVerbs.isEmpty()) {
            Toast.makeText(this, "No hay verbos válidos para guardar", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        lifecycleScope.launch {
            try {
                withContext(Dispatchers.IO) {
                    validVerbs.forEach { (base, past, participle) ->
                        database.verbDao().addVerbToList(listId, base, past, participle)
                    }
                }

                Toast.makeText(
                    this@BulkAddVerbsActivity,
                    "✓ ${validVerbs.size} verbos guardados",
                    Toast.LENGTH_SHORT
                ).show()
